...
public class SMSServiceProviderImpl implements SMSServiceProvider {
	...
	public SMSResponseDto sendSms(String contactNumber, String message) {
		...
		UriComponents sms = UriComponentsBuilder.fromHttpUrl(api)
				...
				.queryParam(SmsPropertyConstant.SMS_MESSAGE.getProperty(), message).build();
		try {
			String smsUri = sms.toUriString().replaceAll("\n", "");
			//logger.info("URL : {} >>>>>> {}", smsUri);
			restTemplate.getForEntity(smsUri, String.class);
